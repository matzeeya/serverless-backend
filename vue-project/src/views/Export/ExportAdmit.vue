      ...
      <thead>
         <tr>
          <td colspan='5' style='text-align:center'>
            <b>รายการแจ้งซ่อมสำเร็จ ปี {{ thisYear }}</b>
          </td>
        </tr>
        <tr>
...
export default {
  ...
  created(){
    ...
    let obj = [];
    let arr = [];
    const docRef = firestore.collection('admits');
    const query = docRef
  ...
  methods: {
    onExport() { // เมื่อกดปุ่มจะทำการสร้างไฟล์ xcel ด้วย xlsx
      ...
      const dataWS = utils.table_to_book(elt,{sheet: 'รายการซ่อมสำเร็จ'});
      writeFileXLSX(dataWS,'รายการซ่อมสำเร็จ '+ this.thisYear +'.xlsx');
    },
